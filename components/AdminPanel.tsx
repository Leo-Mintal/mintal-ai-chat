...
const DEFAULT_ADMIN_PASSWORD =
  (import.meta.env?.VITE_ADMIN_PASSWORD || "liao123...").trim() || "liao123...";
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [quotaSnapshot, setQuotaSnapshot] = useState<UserQuotaSummary | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    setIsVerified(
      window.sessionStorage.getItem(ADMIN_GATE_SESSION_KEY) === "1",
    );
  }, []);
  ...
  const userIdNumber = useMemo(
    () => parsePositiveInteger(userIdInput),
    [userIdInput],
  );
  const dailyLimitNumber = useMemo(
    () => parsePositiveInteger(dailyLimitInput),
    [dailyLimitInput],
  );

  const refreshQuota = useCallback(
    async (targetUserId: number, silent = false) => {
      if (!silent) {
      ...
      try {
        ...
        if (!silent) {
          setFeedback({
            type: "success",
            message: `用户 ${targetUserId} 的当日额度已同步`,
          });
        }
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "额度查询失败，请稍后再试";
        if (!silent) {
        ...
        <div className="bg-white/70 dark:bg-night-card/60 backdrop-blur-md border-[2px] border-white dark:border-white/10 rounded-[40px] p-6 sm:p-8 shadow-soft dark:shadow-night animate-slide-up-bouncy space-y-6">
          {!isVerified ? (
            <>
              <div className="space-y-1">
                <h3 className="text-lg font-extrabold text-warm-800 dark:text-white flex items-center gap-2">
                  <div className="p-1.5 bg-cheese-100 dark:bg-white/10 rounded-lg">
                    <Lock
                      size={18}
                      className="text-cheese-500 dark:text-starlight-400"
                    />
                  </div>
                  请输入管理员密码
                </h3>
              ...
              <div
                className="animate-slide-up-bouncy"
            ...
            <>
              <div className="space-y-2 animate-pop-in">
                ...
                <p className="text-sm text-warm-500 dark:text-slate-400">
                  提交后将调用 PUT /api/v1/llm/quota，按 user_id 更新
                  daily_limit。
                </p>
              </div>

              <div
                className="rounded-[28px] border border-white/70 dark:border-white/10 bg-white/60 dark:bg-night-surface/60 backdrop-blur-sm p-4 space-y-3 animate-slide-up-bouncy"
                style={{ animationDelay: "0.1s" }}
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  >
                    <RefreshCcw
                      size={12}
                      className={isRefreshingQuota ? "animate-spin" : ""}
                    />
                    刷新
                  </button>
                </div>
                ...
                (!userIdNumber || quotaSnapshot.userId === userIdNumber) ? (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs animate-pop-in">
                    ...
                    <div className="rounded-2xl bg-cream-50/80 dark:bg-black/20 border border-cheese-100 dark:border-white/10 px-3 py-2 hover:scale-105 transition-transform duration-300">
                      ...
                      <p className="text-sm font-extrabold text-warm-700 dark:text-white mt-1">
                        {formatResetTime(quotaSnapshot.quotaResetAt)}
                      </p>
                    </div>
                  </div>
                ) : (
                  <p className="text-xs text-warm-500 dark:text-slate-400">
                    输入用户 ID 后点击刷新，可查看当前额度使用情况。
                  </p>
                )}
              </div>
