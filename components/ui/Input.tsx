...
export const Input: React.FC<InputProps> = ({ label, labelAction, error, icon, className = '', ...props }) => {
  return (
    <div className="w-full group relative">
      ...
      <div className="relative transition-all duration-200 hover:-translate-y-px motion-reduce:transform-none">
        <input
          className={`
            ...
            block py-2.5 px-4 transition-all duration-200 shadow-sm placeholder-warm-300 dark:placeholder-slate-500
            ${icon ? 'pl-11' : ''}
            ${error 
        ...
        <div className={`
          absolute right-0 top-0 h-full flex items-center pr-3 pointer-events-none transition-all duration-200
          ${error ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}
        `}>
          {error && <AlertCircle className="text-red-400" size={18} />}
        </div>
      </div>
      {error && (
        <p className="mt-1 ml-1 text-[11px] font-bold text-red-400 animate-pop-in">{error}</p>
      )}
    </div>
  );
};
